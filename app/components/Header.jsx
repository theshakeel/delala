...
export default function Header() {
  ...
  const user = authUser; // ✅ connect to real user
    ...
    <header className="sticky top-0 z-50 bg-[var(--delala-green)] shadow-sm text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <>
            ...
            <div className="fixed top-0 left-0 w-64 h-full bg-white text-gray-800 z-50 shadow-lg flex flex-col">
              ...
              <div className="flex flex-col p-4 gap-2">
                <Link href="/user/settings?page=saved-ads" onClick={() => setMobileMenuOpen(false)}>
                  <MenuItem icon={<Heart size={16} />} text="Saved" />
                </Link>
                <Link href="/user/settings?page=my-messages" onClick={() => setMobileMenuOpen(false)}>
                  <MenuItem icon={<MessageSquare size={16} />} text="Messages" />
                </Link>
                <Link href="/user/settings?page=my-shop" onClick={() => setMobileMenuOpen(false)}>
        ...
        <div className="flex items-center gap-3 relative">
          {user ? (
            <>
              ...
              <div className="hidden sm:flex items-center gap-2">
                ...
                <Link href="/user/settings?page=my-messages">
              ...
              <div className="relative">
                ...
                {menuOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white text-gray-800 rounded-lg shadow-lg overflow-hidden">
                    <MenuItem icon={<Store size={16} />} text="My Shop" as={Link} href="/user/settings?page=my-shop" />
                    <MenuItem icon={<Settings size={16} />} text="Settings" as={Link} href="/user/settings?page=profile" />
                    <MenuItem icon={<Star size={16} />} text="Feedback" as={Link} href="/feedback/11" />
                    <MenuItem
                      icon={<LogOut size={16} />}
                      text="Logout"
                      onClick={() => {
            ...
            <>
              <button onClick={() => openModal("login")} className="text-sm text-white hover:underline">
                Sign In
              </button>
              <span className="text-white/50">|</span>
              <button onClick={() => openModal("register")} className="text-sm text-white hover:underline">
                Register
              </button>
            </>
          )}
          ...
          <button
            onClick={() => {
              if (user) router.push("/user/create-ad");
